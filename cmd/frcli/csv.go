...
var CSVHeaders = "Timestamp,OnChain,Type,Category,Amount(Msat),Amount,Currency,TxID,Reference,BTCPrice,BTCTimestamp,Note"
...
func csv(e *frdrpc.ReportEntry) string {
	...
	return fmt.Sprintf("%v,%v,%v,%v,%v%v,%v%v,%v,%v,%v,%v,%v,%v",
		...
		amountPrefix, e.Fiat, e.FiatCurrency, e.Txid, e.Reference,
		e.BtcPrice.Price, e.BtcPrice.PriceTimestamp, e.Note)
}
