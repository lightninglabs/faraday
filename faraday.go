...
// Stop stops faraday and closes the lnd connection.
func (f *Faraday) Stop() error {
	err := f.StopAsSubserver()
...
// StopAsSubserver stops faraday without closing the lnd connection. This is
// used when faraday runs as a subserver within another process (e.g. litd) that
// manages the shared lnd connection independently.
func (f *Faraday) StopAsSubserver() error {
...
func Main() error {
	...
	err = server.Start()
	if err != nil {
		return fmt.Errorf("error starting faraday: %v", err)
	}
